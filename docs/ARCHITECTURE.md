...
```
                    @trama/core          @trama/tokens
                    (zod only)           (build → CSS + JS)
                          │                       │
              ┌───────────┼───────────┐           │
              ▼           ▼           ▼           ▼
   @trama/projector-web   @trama/projector-embed  (CSS scope import)
      (zustand,
       react 19,                                  ▲
       fizzex 외부)                               │
              │                                   │
      ...
      @trama/host-tiptap ─────────────────────────┘
              │
              ▼
      @trama/host-tiptap-bundle    (Rollup ESM external react/tiptap)

      @trama/ui-primitives — `src/` 는 존재하지만 비어 있음 (현재 미사용)
```

근거: 각 패키지의 `package.json` workspace 의존(`packages/host-tiptap/src/node-view.ts:1`, `packages/projector-embed/src/TramaEmbed.tsx:1-23`, `packages/host-tiptap-bundle/rollup.config.mjs`). `packages/ui-primitives/` 는 `src/` 디렉터리만 있고 `package.json` 도 없음.
...
| `projector-web` | 풀 캔버스 편집기. zustand stores, RAF loop, 인터랙션, 시각 디스패치 | core 와 tokens 만 의존. host 비의존 |
| `projector-embed` | 정적 SVG 렌더 (상호작용 없음, ValueNode 만) | core 와 tokens 만 의존. projector-web 비의존 |
| `host-tiptap` | Tiptap 노드 + DOM 기반 NodeView 어댑터. projector-web 마운트 | Tiptap·React 19 peerDep |
| `host-tiptap-bundle` | Rollup ESM 단일 번들 + 토큰 CSS head 주입 | external: tiptap, react, react-dom |
...
## 5. Projector-Embed (`packages/projector-embed/src/TramaEmbed.tsx`)

정적 SVG 렌더. 인터랙션·상태·시간 흐름 없음.

- `parseTrama` → `documentToModel` → `initializeFromInitialValues` → `propagateOneStep` **1회**. 결과 `values` 가 그대로 SVG 에 박힌다 (`TramaEmbed.tsx:79-98`).
- ValueNode 만 렌더한다. 다른 종류는 무시: `TramaEmbed.tsx:186` `if (!node || !isValueNode(node)) return null;`
- 자체 `registries.ts` 보유 — projector-web 의 모듈 스코프 싱글톤과 *완전히 다른 인스턴스*. 두 패키지의 등록 상태가 분리된다.
- shape/combiner/constant 만 가져오고, NodeKindRegistry 는 default 만 사용. 펄스/cable/skin 시스템 없음.

---

## 6. Host-Tiptap (`packages/host-tiptap/src`)
...
## 7. Host-Tiptap-Bundle (`packages/host-tiptap-bundle/rollup.config.mjs`)
...
- CSS 주입: `projector-web/styles.css` → `@trama/tokens/css` import → postcss-import 플랫화 → 번들 import 시점에 `<head>` 주입. `[data-trama-root]` 스코프 유지.
- 산출물: `host-tiptap-bundle.js` + chunk 트리 + `host-tiptap-bundle.d.ts`. tarball 로 배포.

---

## 8. Tokens (`packages/tokens/build.ts`)
...
### 10.2 도메인-시각 계층 완전 분리

`core` 가 React/zustand/DOM 무의존이라 projector-embed 가 *동일 모델을 정적 1-step 렌더* 로 재사용한다 (`TramaEmbed.tsx:84-88`). 이는 마크다운 fence → 정적 임베드 → 풀 편집기로의 *수준별 렌더링 사다리* 를 가능하게 한다. `rules/concerns/C1-projector-separation.md` 가 이 분리를 강제.

### 10.3 ExecValue 3-채널의 표현력
...
- host-tiptap-bundle 의 external react/tiptap — 호스트 단일 인스턴스 보장.
...
### 11.3 projector-embed 의 좁은 표면

ValueNode 만 렌더 (`TramaEmbed.tsx:186`). Observe/Stock/Generator 등을 정적 렌더해야 하는 사용처 (예: 마크다운 문서에 Sparkline 미리보기) 가 생기면 *두 번째 디스패치 체계* 를 만들거나 projector-web 일부를 끌어와야 한다.

### 11.4 멀티 슬롯 인프라의 적용 범위

`slotIndex/sourceSlotIndex` 와 `OutputSlotSpec` 가 인프라 차원에 있지만 (`model/types.ts:383-389`, `execution/kinds/port-spec.ts`), 실제 멀티 출력 노드는 Stock(3 슬롯), Condition(2 슬롯) 뿐. 향후 N:N 노드가 늘어나면 spawn-policy, socket-registry, cable-points-registry, snap 판정, edge-draft 가 *모든 자리에서 슬롯 인덱스 전파* 를 일관되게 다뤄야 한다 — 현재는 slot 0 폴백 경로가 곳곳에 남아 있어 (`packages/projector-embed/src/TramaEmbed.tsx:157-158`) 회귀 위험.

### 11.5 WrappedValue lifecycle 의 함정
...
ExpressionNode 가 fizzex 패키지에 묶여 있다 (`expression/fizzex-evaluator.ts`). fizzex 가 sibling file 참조라 워크스페이스 외부 사용자 입장에서는 *trama + fizzex 묶음* 을 받아야 한다. host-tiptap-bundle 이 fizzex 를 manual chunk 로 분리했지만 번들 사이즈는 늘어남.

### 11.8 paused-only 편집 제약
