...
```
                    ...
                    (zod only)           (build → CSS + JS)
                          ...
                          ├───────────┬───────────┤
                          ▼           ▼           ▼
              @trama/projector-web   @trama/projector-static  (CSS scope import)
      ...
      @trama/layout — projector-web/static 공용 NodeLayout 계산
      @trama/ui-primitives — `src/` 는 존재하지만 비어 있음 (현재 미사용)
```

근거: 각 패키지의 `package.json` workspace 의존(`packages/host-tiptap/src/node-view.ts:1`, `packages/projector-static/src/TramaStaticView.tsx:1-23`, `packages/host-tiptap-bundle/rollup.config.mjs`). `packages/ui-primitives/` 는 `src/` 디렉터리만 있고 `package.json` 도 없음.
...
## 6. Host-Tiptap-Bundle (`packages/host-tiptap-bundle/rollup.config.mjs`)
