...
| 패키지 | 역할 |
|--------|------|
| [`@trama/core`](packages/core) | 모델·함수·실행·단위·스키마. React 비의존. |
| [`@trama/tokens`](packages/tokens) | JSON 토큰 → TS 상수 + 스코프 CSS (`[data-trama-root]`). 글로벌 `:root`에 절대 깔지 않음. |
| [`@trama/projector-web`](packages/projector-web) | 풀 캔버스 인터랙티브 편집기. |
| [`@trama/projector-static`](packages/projector-static) | 정적 읽기 전용 임베드 (NodeSnapshot 기반 zero-compute SVG). |
| [`@trama/host-tiptap`](packages/host-tiptap) | Tiptap 노드 확장 — `tramaBlock` fence를 NodeView로 마운트. |
| [`@trama/host-tiptap-bundle`](packages/host-tiptap-bundle) | 위 셋을 단일 ESM으로 묶은 파일 — 외부 호스트가 tarball로 소비. |
